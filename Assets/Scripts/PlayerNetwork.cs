...
{    /// <summary>
    ...
    public override void OnNetworkSpawn()
    {
        if (!IsOwner)
        {
            Destroy(transform.GetComponent<Quad>());
            Destroy(transform.GetComponent<InputManager>());
            Destroy(transform.GetComponent<PIDManager>());
        }
    }
    ...
    private void ConsumeState()
    {
        ...
        transform.rotation = Quaternion.Euler(
                Mathf.SmoothDampAngle(transform.rotation.eulerAngles.x, _playerState.Value.Rotation.x, ref _rotVel.x, _cheapInterpolationTime),
                Mathf.SmoothDampAngle(transform.rotation.eulerAngles.y, _playerState.Value.Rotation.y, ref _rotVel.y, _cheapInterpolationTime),
                Mathf.SmoothDampAngle(transform.rotation.eulerAngles.z, _playerState.Value.Rotation.z, ref _rotVel.z, _cheapInterpolationTime)
                );
    }
